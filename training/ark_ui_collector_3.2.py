...
# Windows constants for keyboard hook
WH_KEYBOARD_LL = 13
WM_KEYDOWN = 0x0100
WM_KEYUP = 0x0101

# For low-level keyboard hook
user32 = ctypes.WinDLL('user32', use_last_error=True)

# Function prototype for the hook callback
...
class ARKUICollector:
    ...
    def setup_key_monitor(self):
        """Set up a true low-level keyboard hook that intercepts all key presses system-wide."""
        print("Setting up global low-level keyboard hook...")
        
        # Dictionary to track key states to prevent repeats
        self.key_state = {vk: False for vk in self.vk_codes}
        ...
        def keyboard_callback(n_code, w_param, l_param):
            if n_code >= 0:
                try:
                    # Get the key code from the hook structure
                    kb_struct = ctypes.cast(l_param, ctypes.POINTER(KBDLLHOOKSTRUCT)).contents
                    vk_code = kb_struct.vkCode
                    
                    # Check if it's one of our function keys
                    if vk_code in self.vk_codes:
                        # Key down event
                        if w_param == WM_KEYDOWN and not self.key_state[vk_code]:
                            self.key_state[vk_code] = True
                            key_name = self.vk_codes[vk_code]
                            print(f"Key press detected: {key_name}")
                            # Process in a separate thread to not block the hook
                            threading.Thread(target=self.handle_key, args=(key_name,), daemon=True).start()
                        # Key up event
        ...
        if not self.keyboard_hook:
            ...
            print(f"Failed to set keyboard hook: {error}")
        else:
            print("Low-level keyboard hook installed successfully")
        
        # This background thread just keeps the Python interpreter running
        def msg_loop():
            msg = wintypes.MSG()
    ...
    def stop(self):
        """Stop the collector and unhook keyboard hook."""
        self.is_running = False
