...
class GameWindowSelector:
    """Just finds the ARK window - nothing else."""
    def __init__(self):
        self.monitor = None
        
        # Try to find any ARK window
        try:
            ...
            for title in window_titles:
                try:
                    ...
                    if hwnd != 0:
                        # Just use the full screen monitor instead of window coordinates
                        with mss.mss() as sct:
                            self.monitor = sct.monitors[1]  # Primary display
                        print(f"ARK window found: '{title}'")
                        print(f"Using full screen monitor")
                        return
                except:
                    continue
        ...
        # Always fall back to full screen
        with mss.mss() as sct:
            self.monitor = sct.monitors[1]  # Primary display
            print("Using full screen monitor")
...
class ARKUICollector:
    ...
    def select_game_window(self):
        ...
        self.monitor = selector.monitor
        
        if not self.monitor:
            print("Error: No screen monitor available")
            return False
